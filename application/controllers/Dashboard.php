	...
	public function index() { 
		if (!$this->authenticationlibraries->active()) {
			...
			if ($this->session->type == 'administrator') {
				...
				$data['family'] 	  = FamilyModel::where('nik', $this->session->identity)->count();
				$data['family_child'] = FamilyModel::where('nik', $this->session->identity)->where('relationship', 'CHILD')->count();
				$data['total_homebase'] = ReservationModel::where('nik', $this->session->identity)->where('metode_pemesanan', 'H')->count();
				$data['total_reimbursement'] = ReservationModel::where('nik', $this->session->identity)->where('metode_pemesanan', 'R')->count();
				...
				$data['rsvp_reimbursement_this_month'] = 
					DB::select(
						DB::raw('
							...
							where EXTRACT(MONTH FROM created_at) = EXTRACT(MONTH FROM now())
							and   rsvp.metode_pemesanan = "R"
							and   rsvp.nik = "'. $this->session->identity .'"
							group by month
						')
					);
				$data['rsvp_reimbursement_min1_month'] = 
				...
				$data['rsvp_homebase_last']      = ReservationModel::where('metode_pemesanan', 'H')->where('nik', $this->session->identity)->take(5)->orderBy('id', 'desc')->get();
				$data['rsvp_reimbursement_last'] = ReservationModel::where('metode_pemesanan', 'R')->where('nik', $this->session->identity)->take(5)->orderBy('id', 'desc')->get();
			}
